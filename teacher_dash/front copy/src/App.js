...
function App() {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await axios.get('http://localhost:5001/getAssignments');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5001/createAssignment', {
        title,
        description,
  ...
  const handleEdit = (assignment) => {
    ...
    // Store team sizes but don't allow editing
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5001/updateAssignment/${editingAssignment.assignment_id}`, {
        title,
        description,
        deadline: deadline.toISOString().split('T')[0],
        classes: classes.split(',').map(c => c.trim())
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this assignment?')) {
      try {
        await axios.delete(`http://localhost:5001/deleteAssignment/${id}`);
        ...
        <div className="popup">
          <div className="popup-content">
            ...
            <form onSubmit={handleUpdate}>
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" required />
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" required />
              {/* Display team sizes as read-only */}
